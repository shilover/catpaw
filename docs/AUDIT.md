...
## 10. 第五批改动（打击感 / 节奏与难度 / 每日挑战）

### ① 打击感三件套 — 完成

此前全项目 `shake` / `particles` 零匹配，切割瞬间只有音效和纸芯白边，没有任何物理反馈。

- **纸屑**：`BootScene` 生成一张不规则毛边小纸片贴图，切割时按鱼身颜色染色喷出（普通 9 片、
  完美 16 片），沿刀口方向甩开。用 tween 不用物理体——纯装饰、寿命固定，无需回收清单。
- **顿帧**：完美切割冻结 55ms。**实现上暂停 arcade world + 在场景 `update` 扣真实 delta**，
  而不是改 `time.timeScale`——后者会把负责恢复的定时器一起缩掉，`timeScale = 0` 时永远恢复不了。
  顿帧期间同时冻结单条鱼的倒计时，否则等于白扣玩家时间。
- **震屏**：完美切割 90ms / 强度 0.005，分屏下各用各的相机。

实测：完美切割瞬间 `perfect=1 / frozen=true（剩 38ms）/ physicsPaused=true / shaking=true /
scraps=16`；冻结自行释放后物理恢复；**中途离开场景（`impact.destroy()`）能正确释放共享物理世界**，
不会把它卡在暂停态。

### ② 节奏与难度曲线 — 完成

- **死时间**：切完到下一条鱼原本写死 1000ms（`delayedCall(700)` + `delayedCall(300)`），
  按每局约 20 条鱼算相当于**整局 60 秒里有三分之一在干等**。改为 260 + 140 = 400ms。
  实测连续两次出鱼间隔 **434ms / 386ms**（改前约 1000ms）。
- **难度曲线**：`minElapsed` 由 `0/18/36/52` 改为 `0/14/30/45`，最难的 `Riptide!` 从只存在
  8 秒变成 15 秒。新增单测钉住"最后一档至少有 12 秒"和"每档至少 10 秒"。

> 数值（400ms、0/14/30/45）是按建议值落的，都在 `fishData.js` 顶部集中可调，实际手感请自行试玩后微调。

### ③ 每日挑战 — 完成

无服务器也能做出"明天还想再来"，并且复用了第四批做的分享。

- `utils/random.js`：mulberry32 种子随机 + 按**本地日期**播种（"今天"是玩家体感的今天）。
- `DailyChallengeScene` 继承 `ArcModeScene`，只重写 `createLaneRandom()`。凡是决定"这一局长
  什么样"的随机（鱼种、目标、出生位置、洋流相位、浮动相位）全部改走注入的随机源。
- 独立的当日最佳（`tinyfish.daily`，跨天自动失效），主菜单第 4 个模式按钮显示今日最佳，
  分享文案带上日期（否则分数无从比较）。

实测：**两个互相独立的浏览器会话，进入每日挑战后前 6 条鱼的鱼种 / 目标百分比 / 出生坐标完全一致**；
同时确认街机模式仍然是随机的（两次运行序列不同）。
...
## 11. 第六批改动（新手引导 + 理想切线教学）

### ① 理想切线 — 完成

游戏最核心的学习障碍：**目标百分比是个抽象数字，玩家很难把它换算成一条线的位置**。

- 切完之后画一条**与玩家实际切线平行**的绿色虚线，标出目标真正在哪（`ui/cutGuide.js`）。
  平行而非任意角度，是为了让教学信息是"你差了这么多"，而不是"你该换个角度切"。
- 几何在 `polygonCut.js` 的 `idealChordDistance()`：把椭圆缩放成单位圆后面积比不变、直线仍是
  直线，而圆上的答案只跟距离有关、与方向无关，再按同一个长度缩放回去。
  **单测覆盖 7 个角度 × 5 个目标的往返一致性（误差 < 0.3%）**，另有与既有水平解一致性、
  50% 恒过圆心两条。
- 三态设置 `auto` / `on` / `off`（默认 `auto`）：`auto` 只在生涯切鱼数 < 30 时显示；
  完美切割不显示（没什么可教的）。设置放在帮助弹窗里，避免设置行继续膨胀。

实测四种组合全部正确：`auto`+新手→显示、`auto`+老手(120刀)→不显示、`on`+老手→显示、
`off`+新手→不显示；三态按钮循环 `auto → on → off → auto` 正确。

### ② 新手引导 — 完成

不是文字弹窗，而是**用真实玩法分步教学**。

- `TutorialScene` 跑的就是真实的 `GameplayLane`，只是关掉了单条鱼倒计时（`timedFish: false`）
  和自动出鱼，并强制打开理想切线。**每步等玩家做到才推进，不用计时器赶人**。
- 5 步：划一刀 → 对准目标 → 看懂绿线 → 抓章鱼 → 连击。第 4 步为了教"点章鱼"这个操作，
  新增 `spawnTutorialOctopus()` 直接给一只，不要求先切出完美。
- 首次启动由 `BootScene` 路由到 `Tutorial`（菜单对还不懂玩法的人没有意义），
  完成/跳过后写入 `tinyfish.tutorialDone`；帮助弹窗里可重玩。

实测：全新档案首次启动确实进入 `Tutorial`；5 步各由正确的事件推进（切、切、切、点章鱼、切）；
完成后落盘 `tutorialDone=true` 并转入 `MainMenu`；已完成的档案再次启动直接进菜单。

### ③ 顺带修的布局问题

帮助弹窗加了两个控件后，它们把正文和"关闭"按钮压住了。**没有继续手调数值，而是把
`showPopup` 改成按内容自动算高度**——正文先测量，按钮从底部往上堆。用运行时包围盒做了
重叠检测：中英双语均 **0 处重叠**，且面板完整落在画布内（中文 123~589，英文 111~602）。
...
### ① 鱼种体型 `bodyRatio` — 完成（做关卡的前提）

设计关卡时核实发现的既有问题：**所有鱼种的切割椭圆都是 140×96**（`rx = 贴图宽 × 0.42`），
只有整体大小差 17%。也就是说圆滚滚的河豚和扁平的魔鬼鱼**在几何上是同一道题**，美术在骗玩家。

- 给每个鱼种加 `bodyW`/`bodyH`，数值对着 `BootScene` 实际画出来的轮廓量。
- 长宽比范围 **0.81（神仙鱼，竖长）~ 1.71（魔鬼鱼，宽扁）**，河豚正好 1.00 是正圆。
- **同时按等面积重算了各鱼的 `size`**（面积极差 53% → **0%**）：否则"体型关"会退化成考鱼的大小，
  大鱼天然更好切。
- 魔鬼鱼原本画出来是 88×80，并不扁，第 2 关就缺一条宽扁鱼。把翼展改宽压扁，让美术和名字对上
  （**这是超出"加字段"的美术改动，明确标注**）。
- 用叠加椭圆的截图逐条核对了几何与美术是否吻合。

### ② 七关关卡系统 — 完成

按"**七个维度就七关，一关一维度**"落地，不注水：

| 关 | 维度 | 隔离手段 |
|---|---|---|
| 1 | 精准 | 单一鱼种、无风、宽裕时间，目标由 50% 走向 30% |
| 2 | 体型 | 五种鱼**轮转**（非随机）、目标恒定 25% |
| 3 | 角度 | **只能竖切**，用正圆的河豚以免形状提供方向暗示 |
| 4 | 洋流 | 风幅 88（全场最强） |
| 5 | 快刀 | 单鱼 2.4 秒（全场最短），目标放宽 |
| 6 | 稳定 | **按最高连击评星**，不看分数 |
| 7 | 余量 | 鱼缩到 0.5 倍 |

- 每关固定种子；星级 1 星通关 / 3 星精通；过 1 星解锁下一关。
- 关卡规则通过 `GameplayLane` 的 `rules` 注入，无尽模式传空对象照走难度分段——**一处开关，两套玩法**。
- 角度限制**拒绝这一刀并提示，不消耗这条鱼**（实测：横切 `cutCount=0`、鱼还在、提示出现；
  竖切 `cutCount=1`）。
- 成长按决策走"**解锁内容**"而非数值：关卡顺序解锁，不提供任何放宽判定的道具——
  这游戏的乐趣就是精度本身，能用资源换宽容度等于把技能架空。

实测：七关的规则（时长/鱼种池/单鱼时间/风幅/角度/鱼缩放）逐关读取核对全部正确；
通关落盘星级、**更差的重玩不会扣掉已得星级**；选关界面锁定态与解锁态渲染正确。

### 第八批：启动优化 / 合作模式重做 / 生存模式

#### ① 启动优化 — 完成

用 **4× CPU 降频**（≈中端手机）在生产构建上量化，从导航开始计时：

| | 改前 | 改后 |
|---|---|---|
| 首次可见内容 | **3.35s（纯黑屏）** | **1.27s（动画 + 真实进度条）** |
| 游戏就绪 | 3.35s | **2.35s** |

三项具体改动：
1. **`paperize()` 的压边距离图只在 `edgeShade > 0` 时才算。** 全屏背景传的是 `edgeShade: 0`，
   却照样跑 4 趟 × 2 次扫描 × 92 万像素算一张距离图，然后乘以 0 扔掉——约 740 万次纯浪费迭代。
2. **音乐床移出启动路径 + 降采样到 11025Hz。** 8 秒 pad 原本占 845ms（4× 降频下），是全场最贵的
   单项；慢速铺底音没有高频，降采样无听感损失。改为场景起来 120ms 后按需合成。
3. **加载遮罩写在 `index.html` 标签里**（JS 执行前就绘制），`BootScene` 改为**每帧只做一件生成
   工作**，于是进度条是真实进度而不是假动画（实测 8% → 100%）。

> 副作用值得记一笔：`game.isRunning` 为真**不再等于可玩**——此时遮罩还盖着画布。
> 21 个自动化探针全部改为等待 `!document.getElementById('boot-splash')`。

#### ② 合作模式改成真合作 — 完成

核实到的**真实设计缺陷**：`onEitherResolved` 把先手玩家的结果直接复制给另一半，
**第二个玩家的技术完全不影响任何事**，只有点章鱼算他的。两人同屏，其中一个实质是观众。

改为：**双方都必须自己切**，各自按自己的准度计分；两刀落进同一个 5% 分档给双方
"默契奖励 +60"。团队总分改为两边直接相加（复制结果的年代这样加会把每分算两遍）。

实测：P2 切完之后 `scoreA=0 / cutsA=0 且 P1 的鱼还在`（改前会被连带判定）；
P1 再切之后两边各 90 分、各含 60 的默契奖励。

#### ③ 生存模式 — 完成

补上"全游戏没有失败状态"这个缺口：Arc / 每日 / 关卡都是跑满时间就结束，切得再烂也不会提前结束，
**冲高分毫无紧张感**。

- 三条命、无时限，漏切一次扣一条命；难度**无上限地爬**（单鱼时间 5.0→1.5 秒、洋流 0→130，
  180 秒走完），因为有命数兜底就不需要难度天花板。
- 顶栏计时改为**已坚持时长**，独立的最佳纪录同时记录分数与时长。

实测：3→2→1→0 扣命与红心显示正确，归零后转入结算；成绩落盘且更差的重玩不覆盖。

### 第七批之后仍未做

- 真实美术资源 / Preloader（已明确选择继续程序化生成，此项视为不再需要）
- 同屏多条鱼（`GameplayLane.currentFish` 是单数，改多鱼是结构性重构，已建议延后）
- 每日任务、货币 / 皮肤、在线排行榜、错误上报
- 反馈飘字的 Text 对象池（实测每刀创建 3 个 Text，各自带一张 canvas；优先级低）
- 对战模式的干扰手段仍只有泼墨 / 抖动两种
- 每日任务、货币 / 皮肤（后者已明确建议暂缓：缺内容支撑会变成空壳）
- 在线排行榜（需服务器 + 反作弊，本地分数是明文 localStorage）
