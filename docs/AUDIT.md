...
### 第九批：Text 对象池 + 对战第三种干扰

#### ① 反馈文字对象池 — 完成

上一批量到"每刀创建 3 个 Text"（Phaser 的 Text 每个自带一张 canvas）。这次按调用点归因，
逐项处理：

| 来源 | 改前（10 刀） | 改后 |
|---|---|---|
| 飘字反馈 | 每刀 1 个 | **总共 2 个**，之后全部复用 |
| 鱼名标签 | 每条鱼 1 个 = 10 | **0**（从 `CuttableFish` 移到 lane，一个复用的 Text 跟随当前鱼） |
| 理想切线标签 | 每刀 1 个 | 保留 —— 只在教学期存在（生涯 30 刀后 `auto` 自动关闭） |

也就是说**老玩家稳态下每刀 0 次 Text 分配**。实测鱼名标签跟随漂移的鱼、切鱼后隐藏。

#### ② 对战模式第三种干扰 — 完成

此前只有泼墨（攻击视野）和抖动（攻击目标稳定性）两种，且都只看单刀结果。新增
**"目标隐藏"——攻击信息**：藏掉对手刻度条上的目标箭头，刻度仍在，逼他凭记忆下刀。

三档按难度递进，并且**最强的一档挂在连击上**，让连击系统在对战里也有意义（此前对战只看分数，
连击几乎无关）：
...
实测：连击 1 时完美只泼墨、箭头仍在；连击 3 时箭头被隐藏且计时器就位；差一档仍然只抖动；
隐藏会自行到期恢复。
...
这些是提过方案、评估过、**由项目方决定不做**的，不要再作为建议重复提出：

| 项 | 决定 | 理由 |
|---|---|---|
| **同屏多条鱼** | ❌ 不做 | `GameplayLane.currentFish` 是单数，改多鱼要动切割判定、倒计时、连击与碎片管理，属结构性重构。收益不确定，成本明确 |
| **真实美术资源** | ❌ 不做 | 已选择继续程序化生成 + 纸感；玩法本身就是"切纸"，风格与机制契合，且零授权风险、零包体增长 |
| **数值型成长（道具/放宽判定）** | ❌ 不做 | 这游戏的乐趣就是精度本身，能用资源换宽容度等于把技能架空。成长走"解锁内容" |
| **自动部署** | ❌ 不做 | CI 只跑测试与构建，不发布任何东西 |

> 说明：本节替换了此前分散在第 6~8 节末尾的多份"仍未做"列表——那些列表里的成就系统、
> 鱼种 `baseScore` 差异化、关卡系统、新手引导等条目都已经完成，继续留着会误导。
