...
│   │   ├── fishData.js       ← 鱼种表（含体型/价值/稀有度）、目标规则、评分、连击、难度分段
│   │   ├── levels.js         ← 七关定义 + 星级判定 + 解锁规则（纯函数，有单测）
...
**七个技能维度，就七关，一关一维度**——精准 / 体型 / 角度 / 洋流 / 快刀 / 稳定 / 余量。
不靠"这关要 800 分下关要 1000 分"堆数量，那只是同一道题换数字。

- 每关**隔离自己那个维度**：其余的变量全部压平。所以卡关的人能立刻知道是哪项能力不行。
  单测钉住了这条（洋流关必须是风最大的、快刀关必须是钟最短的、只有余量关能缩小鱼）。
- 每关**固定种子**，所有玩家拿到同一套鱼，成绩才可比。
- 星级：1 星＝通关（门槛低，不卡流程），3 星＝精通。**过 1 星就解锁下一关**——闸门是用来排课的，
  不是用来逼人刷的。稳定关按最高连击评星而不是分数，因为它考的就是不掉链子。
- 关卡规则通过 `GameplayLane` 的 `rules` 参数注入（`species` / `targets` / `wind` / `cutTime` /
  `cutAngle` / `fishScale`），**无尽模式传空对象、走难度分段**。加新规则改这一处即可。
- 角度限制的处理方式是**拒绝这一刀并给提示，不消耗这条鱼**——教学要立刻生效，代价只是时间。

### 鱼的体型（`bodyW` / `bodyH`）

⚠️ **切割椭圆来自鱼种自己的 `bodyW`/`bodyH`，不再来自贴图尺寸。** 以前所有鱼共用 140×96 的椭圆，
于是"圆滚滚的河豚"和"扁平的魔鬼鱼"在几何上是同一道题、只是换了张画——美术在骗玩家。

- 数值是**对着 BootScene 实际画出来的轮廓量的**，改绘制就要同步改这两个数。
- **各鱼种的椭圆面积必须保持基本相等**（靠调 `size` 拉平，现为 0% 极差），否则"体型关"就变成
  考鱼的大小而不是形状。有单测钉住面积极差 ≤ 6%、长宽比极差 ≥ 1.8 倍。
- `ringRadius` 和 `getSpawnBounds` 仍然用贴图尺寸——前者要圈住整条画出来的鱼，后者要保证整个
  精灵不出界，跟切割椭圆是两回事。
